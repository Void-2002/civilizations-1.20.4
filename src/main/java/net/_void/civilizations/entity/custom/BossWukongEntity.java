...
public class BossWukongEntity extends AnimalEntity {

    private static final TrackedData<Boolean> ATTACK = DataTracker.registerData(BossWukongEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    private static final TrackedData<Boolean> SPELL = DataTracker.registerData(BossWukongEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    ...
    private static TrackedData<Integer> Z = DataTracker.registerData(BossWukongEntity.class, TrackedDataHandlerRegistry.INTEGER);
    ...
    private void setupAnimationStates() {
        ...
        if(this.isSpelling() && spellAnimationTimeout <= 0) {
            spellAnimationTimeout = 20;
            spellAnimationState.start(this.age);
        } else {
            --this.spellAnimationTimeout;
        }
        if(!this.isSpelling()) {
            spellAnimationState.stop();
        }
    }
    ...
    protected void initGoals() {
        ...
        this.goalSelector.add(1, new BossWukongAttackGoal(this));
    ...
    public void setSpelling(boolean spell){
        this.dataTracker.set(SPELL, spell);
    }

    public boolean isSpelling() {
        return this.dataTracker.get(SPELL);
    }
        ...
        if(damageTaken >= 230){
            ...
            for(int i=-5;i<=5;i+=10){
                for(int j=-5;j<=5;j+=10){
                    BlazeEntity customEntity = ((EntityType<BlazeEntity>) EntityType.get("minecraft:blaze").get()).create(this.getWorld());
                    customEntity.updatePosition(this.getX() + i, this.getY(), this.getZ() + j);
    ...
    protected void initDataTracker() {
        ...
        this.dataTracker.startTracking(SPELL,false);
    ...
    public void onDeath(DamageSource damageSource) {
        ...
        ItemEntity itemEntity = new ItemEntity(this.getWorld(),this.dataTracker.get(X) + 0.5, this.dataTracker.get(Y), this.dataTracker.get(Z) + 0.5, new ItemStack(ModItems.WUKONG_SWORD));
